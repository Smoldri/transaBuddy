...
public class OrderService {
    ...
    public List<OrderInfo> findActiveOrdersByUserId(Integer userId) {
        ...
        for (OrderInfo orderInfo : orderInfos) {
            if (userId.equals(orderInfo.getCourierUserId())) {
                List<OrderInfo> courierActiveOrders = findAllCourierActiveOrders();
                for (OrderInfo courierActiveOrder : courierActiveOrders) {
                    if (userId.equals(courierActiveOrder.getCourierUserId())) {
                        activeOrders.add(courierActiveOrder);
                    }
                }
            } else if (userId.equals(orderInfo.getSenderUserId())) {
                List<OrderInfo> senderActiveOrders = findAllCourierActiveOrders();
                for (OrderInfo senderActiveOrder : senderActiveOrders) {
                    if (userId.equals(senderActiveOrder.getCourierUserId())) {
                        activeOrders.add(senderActiveOrder);
                    }
                }
//                activeOrders = orderInfos.stream().
